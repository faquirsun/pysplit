...
class SeisPick(qt.QMainWindow):
    ...
    def loadCatalogue(self):
        ...
        if not data_dir.exists() and meta_dir.exists():
            msg = "The directory you have chosen is invalid, please try again."
        ...
        try:
            ...
            msg = "Unable to find file containing the metadata for this catalogue.\n"
    ...
    def createCatalogue(self, params=None, new=False):
        ...
        if params["catalogue_type"] == "teleseismic":
            msg = "Creating an instance of TeleseismicCatalogue..."
            ...
            msg = "Creating an instance of LocalCatalogue..."
            self.uiStatusBar.showMessage(msg)
    ...
    def loadArrivals(self):
        ...
        if arrivals:
            ...
            msg = "Arrivals file does not exist - attempting to generate one..."
            self.uiStatusBar.showMessage(msg)
    ...
    def plotCatalogueMap(self, replot=False):
        ...
        if replot:
            ...
            if self.catalogue.catalogue_type == "local":
                self.catalogue.plot(map_widget=self.uiMapMpl, receivers=True)
    ...
    def sourceSelect(self, index):
        ...
        if not waveform:
            ...
            msg = "No archive data available for this arrival."
...
class SplittingAnalysisWindow(qt.QMainWindow):
    ...
    def plotHodograms(self):
        ...
        self.event.plotHodogram(self.uiHodogram2Mpl.canvas.ax,
                                phi=self.fast_direction, dt=self.delay_time)
...
class PickingWindow(qt.QMainWindow):
    ...
    def __init__(self, catalogue, source=None, receiver=None, filt=None):
        ...
        if receiver is None and source is not None:
        ...
        elif receiver is not None and source is None:
        ...
        elif receiver is not None and source is not None:
            self.sr_pairs.append([source, receiver])

        sr = self.sr_pairs[0]
        self.current_sr = psm.SourceReceiverPair(self.catalogue_path,
                                                 sr[0], sr[1])
    ...
    def _onMove(self, event):
        ...
        if event.inaxes not in [c1_canvas.ax, c2_canvas.ax, c3_canvas.ax]:
            return
    ...
    def _onClick(self, event):
        ...
        if event.key == "control":
            if event.button == 1:
                color = "green"
                pick_type = "wb"
                self.current_sr.addPick(info_type="window",
                                        value=adjusted_xdata,
                                        pick_type="wb")
            elif event.button == 3:
                color = "green"
                pick_type = "we"
            ...
            if event.button == 1:
                ...
                pick_type = self.pick_type
                self.current_sr.addPick(info_type="pick",
                                        value=adjusted_xdata,
                                        pick_type=self.pick_type)
        ...
        c1_line = c1_canvas.ax.axvline(adjusted_xdata, linewidth=1,
                                       color=color, animated=True)
        c2_line = c2_canvas.ax.axvline(adjusted_xdata, linewidth=1,
                                       color=color, animated=True)
        c3_line = c3_canvas.ax.axvline(adjusted_xdata, linewidth=1,
                                       color=color, animated=True)
    ...
    def plotTraces(self, replot=False):
        ...
        # Plot the traces
        self.current_sr.plotTraces([c1_canvas.ax, c2_canvas.ax, c3_canvas.ax],
                                   lims=self.lims)
    ...
    def rejectTrace(self):
        ...
        if self.current_sr is not None:
            ...
            self.catalogue.removeArrival(self.current_sr.source.sourceid,
                                         self.current_sr.receiver.receiverid)
