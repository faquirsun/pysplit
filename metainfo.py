...
class SourceReceiverPair(object):
    ...
    This class is used to represent the waveforms of a particular event
    ...
    def __init__(self, catalogue_path, source, receiver):
        ...
        self.file_name = self.file_format.format(source.sourceid,
                                                 receiver.station)
    ...
    def correctSplitting(self, phi, dt):
        ...
        lagged_components = [rotated_components[0],
                             np.roll(rotated_components[1],
                             int(dt / self.delta))]
...
class Network(object):
    ...
    def filterReceivers(self, arrivals):
        available = arrivals.drop_duplicates(subset="receiverid",
                                             keep="first").receiverid.values
